...
//
{
  ...
  for ( i = 2; i <= n; i++ )
      /*there was a bug here where the loop ran until i<=n. This meant that memory
      which was never allocated was being created for n-th iteration */
  {
    x[i] = x[i-1] + x[i-2];
    cout << "  " << i << "  " << x[i] << "\n";
  }

  free(x); // We should use free when malloc is used.
